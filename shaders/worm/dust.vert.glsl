...
layout (location = 5) in vec4 phong;
...
uniform float layer; // Index of the dust layer


void main()
{
	// The position of the vertex in the world space
	vec4 position = model * vec4(vertex_position, 1.0) + vec4(pos, 1.0);
	...
	fragment.alpha = mod(time, 1);
	// TODO
	fragment.uv = vertex_uv + time / 40; // we move the texture
